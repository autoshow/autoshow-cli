...
7. Tier breakdown uses `tierMetric: "balanced-overall"` and splits the overall ranking into equal thirds; if the provider count is not divisible by three, the remainder is assigned to Tier 3.
...
JSON report uses `overall`, `local`, and `cloud` top-level keys, each containing `count` and `providers` arrays. It also includes `tierMetric`, `tierSplit`, and `tiers` top-level fields. Each provider object includes backward-compatible `score` fields plus `overallScore`, `overallRank`, `overallComponents`, and `overallTier`.
