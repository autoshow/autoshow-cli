...
Use this skill against one AutoShow video run directory that contains:

1. `run.json` with `kind: "video"` and a `metadata.video[]` array listing each provider entry
2. Video files in the run directory matching `metadata.video[].videoFileName`

The run directory does not use a `providers/` subdirectory. Each video entry is a flat file in the root of the run directory, identified by its `videoFileName` field.
...
4. **Caveats** -- state which metrics were and were not available; acknowledge that video quality cannot be directly assessed
...
7. Artifact files are verified and listed, but file size, duration, dimensions, format, bitrate, and quality are not scoring inputs.
...
1. `run.json` metadata
2. Video files on disk (for existence and file size context)

## Helper Artifact Guidance

`build_evaluation_packet.ts` emits a temporary evidence packet. Prefer writing it outside the run directory or deleting it after use so the final directory contains only the three requested deliverables plus original video files.
