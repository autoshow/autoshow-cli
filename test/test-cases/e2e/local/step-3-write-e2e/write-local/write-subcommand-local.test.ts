...
describe('write subcommand with llama', () => {
  ...
  budgetedTest('write-llama-qwen3-0.6b', 'write https://ajc.pics/autoshow/examples/1-audio.mp3 --llm llama=ggml-org/Qwen3-0.6B-GGUF', async () => {
    await stopLlamaServer()

    const result = await runCommand(
      ['src/cli/create-cli.ts', 'write', STABLE_EXAMPLE_AUDIO_URL, '--llm', 'llama=ggml-org/Qwen3-0.6B-GGUF'],
      {
        env: {
  ...
  budgetedTest('write-llama-qwen3-0.6b', 'write https://ajc.pics/autoshow/examples/1-audio.mp3 --llm llama=ggml-org/Qwen3-0.6B-GGUF --prompt shortSummary longSummary', async () => {
    await stopLlamaServer()
    await cleanupTestOutput(STABLE_EXAMPLE_AUDIO_TITLE)

    const result = await runCommand(
      ['src/cli/create-cli.ts', 'write', STABLE_EXAMPLE_AUDIO_URL, '--llm', 'llama=ggml-org/Qwen3-0.6B-GGUF', '--prompt', 'shortSummary', 'longSummary'],
    )
