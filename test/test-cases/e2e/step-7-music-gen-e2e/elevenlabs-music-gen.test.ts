...
budgetedTest('music-pipeline-elevenlabs-music_v1', 'write with elevenlabs music pipeline writes music artifacts and metadata', async () => {
  ...
  const result = await runCommand(
    ['src/cli/create-cli.ts', 'write', 'https://ajc.pics/autoshow/examples/1-audio.mp3', '--llama', 'ggml-org/gemma-3-270m-it-GGUF', '--elevenlabs-music', 'music_v1', '--music-duration', '3'],
  )
