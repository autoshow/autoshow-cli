...
test('default transcribe processes local audio', async () => {
  await cleanupTestOutput(STABLE_LOCAL_AUDIO_TITLE)
...
for (const modelCase of [
  ...
  test(`whisper ${modelCase.model} model transcribes local audio`, async () => {
    await cleanupTestOutput(STABLE_LOCAL_AUDIO_TITLE)
...
test('split mode processes audio in segments', async () => {
  await cleanupTestOutput(STABLE_LOCAL_AUDIO_TITLE)
