...
describe('write subcommand with llama', () => {
  ...
  test('write input/1-audio.mp3 --llama ggml-org/Qwen3-0.6B-GGUF', async () => {
    await stopLlamaServer()
  ...
  test('write input/1-audio.mp3 --llama ggml-org/Qwen3-0.6B-GGUF --prompt shortSummary longSummary', async () => {
    await stopLlamaServer()
    await cleanupTestOutput(STABLE_LOCAL_AUDIO_TITLE)
...
describe('write subcommand with document input', () => {
  ...
  test('write input/1-document.pdf --llama ggml-org/Qwen3-0.6B-GGUF', async () => {
    await stopLlamaServer()
    await cleanupDocOutput()
