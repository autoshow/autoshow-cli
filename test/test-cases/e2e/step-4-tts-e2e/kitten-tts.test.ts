...
describe("kitten-tts", () => {
  ...
  describe("tts command", () => {
    ...
    const kittenModelCases = [
      { model: 'kitten-tts-micro', speaker: 'Bella' },
      { model: 'kitten-tts-mini', speaker: 'Luna' },
      { model: 'kitten-tts-nano', speaker: 'Rosie' },
      { model: 'kitten-tts-nano-0.8-int8', speaker: 'Hugo' },
    ] as const

    for (const kittenModelCase of kittenModelCases) {
      test(`${kittenModelCase.model} with --tts-speaker ${kittenModelCase.speaker} generates speech.wav`, async () => {
        await cleanupTestOutput(STABLE_TTS_MD_TITLE)
