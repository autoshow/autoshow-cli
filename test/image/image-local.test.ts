...
const cliCommands = [
  { 'sdcpp-default': 'npm run as -- image generate --prompt "A beautiful sunset" --service sdcpp' },
]

test('CLI image local tests', { concurrency: 1 }, async (t) => {
  ...
  for (const commandObj of cliCommands) {
    ...
    await t.test(`Local Image: ${testName}`, { concurrency: 1 }, async () => {
