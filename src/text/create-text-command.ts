...
import { createJsonOutput, setJsonError, outputJson, EXIT_USAGE, type TextJsonOutput } from '@/utils'
...
export function validateCommandInput(options: ProcessingOptions): {
  ...
  if (selectedActions.length > 1) {
    err(`Error: Multiple input options provided (${selectedActions.join(', ')}). Please specify only one.`, undefined, EXIT_USAGE)
  }
  ...
  if (selectedLLMs.length > 1) {
    err(`Error: Multiple LLM options provided (${selectedLLMs.join(', ')}). Please specify only one.`, undefined, EXIT_USAGE)
  }
  ...
  if (options.elevenlabs && options.minimax) {
    err('Cannot use both --elevenlabs and --minimax. Please specify only one music service.', undefined, EXIT_USAGE)
  }
  ...
  if (musicGenre) {
    ...
    if (!validGenres.includes(musicGenre)) {
      err(`Invalid --${musicService} genre: ${musicGenre}. Valid options: ${validGenres.join(', ')}`, undefined, EXIT_USAGE)
    }
    if (!llmServices) {
      err(`--${musicService} requires an LLM option (--chatgpt, --claude, or --gemini)`, undefined, EXIT_USAGE)
    }
  }
...
export async function processCommand(
  ...
  const jsonBuilder = createJsonOutput<TextJsonOutput>('text')
  ...
  if (!action && !options.feed) {
    setJsonError(jsonBuilder, 'No action specified (e.g., --video, --rss, --feed)')
    outputJson(jsonBuilder)
    err('Error: No action specified (e.g., --video, --rss, --feed). Use --help for options.', undefined, EXIT_USAGE)
  }
  ...
  if (options.feed && !action) {
    ...
    jsonBuilder.output.data = {
      inputType: 'feed',
      inputPath: options.feed as string
    }
    outputJson(jsonBuilder)
    exit(0)
  }
  ...
  if (action === 'rss' && options.feed) {
    ...
    jsonBuilder.output.data = {
      inputType: 'rss',
      inputPath: Array.isArray(options.rss) ? options.rss.join(', ') : (options.rss || '')
    }
    outputJson(jsonBuilder)
    exit(0)
  }
  ...
  try {
    ...
    jsonBuilder.output.data = {
      inputType: action || 'unknown',
      inputPath: action ? String(options[action] || '') : '',
...
export const createTextCommand = (): Command => {
  const textCommand = new Command('text')
    ...
    .option('-r, --rss [rssURLs...]', 'Process one or more podcast RSS feeds (optional when using --feed)')
    ...
    .option('-F, --feed <feedFile>', 'Process workflow feed file (e.g., "01-ai-feeds.md") from input/workflows/feeds')
    .option('-w, --whisper [model]', 'Use Whisper.cpp for transcription with optional model specification (e.g., base, large-v3-turbo)')
    .option('-W, --whisper-coreml [model]', 'Use Whisper.cpp (CoreML) for transcription with optional model specification (e.g., base, base.en, large-v3-turbo)')
    ...
    .option('-l, --last <number>', 'Number of most recent items to process (overrides --order)', parseInt)
    .option('-O, --order <order>', 'Specify the order for RSS feed and channel processing (newest or oldest)')
    ...
    .option('-i, --input-dir <directory>', 'Input directory for files (default: input)')
    .option('-o, --output-dir <subdirectory>', 'Output subdirectory within output/ (optional)')
  ...
  textCommand.addHelpText('after', `
Examples:
  $ autoshow-cli text --video "https://www.youtube.com/watch?v=MORMZXEaONk"
  $ autoshow-cli text --file ./input/audio.mp3 --whisper --chatgpt --prompt summary
