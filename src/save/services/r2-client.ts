...
import { getR2ApiToken } from './r2-token-manager'
...
export class R2Client {
  ...
  constructor(accountId: string) {
    ...
    this.baseUrl = `https://api.cloudflare.com/client/v4/accounts/${accountId}/r2/buckets`
  }
  ...
  async createBucket(bucketName: string): Promise<boolean> {
    const p = '[save/services/r2-client]'
  ...
  async headBucket(bucketName: string): Promise<boolean> {
    ...
    try {
      ...
      const response = await fetch(`${this.baseUrl}/${bucketName}`, {
        method: 'GET',
        headers
      })
  ...
  async putObject(bucketName: string, key: string, body: Buffer | string): Promise<boolean> {
    const p = '[save/services/r2-client]'
    ...
    try {
      ...
      const uploadUrl = `${this.baseUrl}/${bucketName}/upload`
      ...
      if (!response.ok) {
        const alternativeUrl = `${this.baseUrl}/${bucketName}/objects/${key}`
  ...
  async getObject(bucketName: string, key: string): Promise<string | null> {
    ...
    try {
      ...
      const objectUrl = `${this.baseUrl}/${bucketName}/objects/${key}`
  ...
  async putBucketVersioning(bucketName: string, enabled: boolean): Promise<boolean> {
    const p = '[save/services/r2-client]'
    l.dim(`${p} Setting versioning for bucket: ${bucketName} to ${enabled ? 'enabled' : 'disabled'}`)
    return true
  }
  
  async putBucketLifecycle(bucketName: string, days: number): Promise<boolean> {
    const p = '[save/services/r2-client]'
  ...
  async healthCheck(bucketName: string): Promise<boolean> {
    const p = '[save/services/r2-client]'
    ...
    try {
      ...
      const writeSuccess = await this.putObject(bucketName, testKey, testContent)
      if (!writeSuccess) {
      ...
      const readContent = await this.getObject(bucketName, testKey)
      ...
      const deleteSuccess = await this.deleteObject(bucketName, testKey)
      if (!deleteSuccess) {
