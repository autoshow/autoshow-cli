...
export const createVideoCommand = (): Command => {
  ...
  video
    ...
    .action(() => {
      ...
      l.dim(`${p} `)
      l.dim(`${p} Google Veo models (cloud-based, requires GEMINI_API_KEY):`)
      ...
      l.dim(`${p} `)
      l.dim(`${p} Runway models (cloud-based, requires RUNWAYML_API_SECRET):`)
      ...
      l.dim(`${p} `)
      l.dim(`${p} Note: All models run on cloud servers and require API keys.`)
    })
