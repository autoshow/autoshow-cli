...
export const lyricsCommand = defineCommand({
  name: 'lyrics',
  description: 'Create a lyric video from local audio with Whisper captions',
  flags: lyricsFlags,
  help: {
    examples: [
  ...
  const batch = ctx.flags.batch === true
  const audioFlag = typeof ctx.flags.audio === 'string' ? ctx.flags.audio : undefined
  const captionsFlag = typeof ctx.flags.captions === 'string' ? ctx.flags.captions : undefined
  const modelRaw = typeof ctx.flags.model === 'string' ? ctx.flags.model : 'large-v3-turbo'
  const font = typeof ctx.flags.font === 'string' && ctx.flags.font.trim().length > 0 ? ctx.flags.font : 'DejaVu Sans'
  const keepTmp = ctx.flags['keep-tmp'] === true
