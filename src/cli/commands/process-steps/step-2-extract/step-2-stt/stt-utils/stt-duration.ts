...
import { DOCUMENT_EXTENSIONS, IMAGE_EXTENSIONS } from '~/cli/commands/process-steps/step-1-download/targets/target-utils'
...
export const resolveSttInputDurationSeconds = async (
  ...
  if (!isLikelyUrl(input)) {
    ...
    if (!exists) {
      throw CLIUsageError(`Input does not exist: ${input}. Run: bun as help extract`)
    }
