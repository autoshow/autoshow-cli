...
export const runGlmVideoGen = async (
  prompt: string,
  outputDir: string,
  options: { model: GlmVideoModel, durationSeconds?: number | undefined, size?: string | undefined, aspectRatio?: string | undefined }
  ...
  const requestBody = options.model === 'cogvideox-3'
    ? {
        model: options.model,
        prompt,
