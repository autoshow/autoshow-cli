...
export const setupMinimaxMusicGen = async (): Promise<void> => {
  ...
  if (apiKey) {
    l.write('success', 'MINIMAX_API_KEY found — MiniMax music generation ready')
  } else {
    l.warn('MINIMAX_API_KEY not set — MiniMax music generation will not work until set')
    l.write('info', 'Set MINIMAX_API_KEY environment variable to use MiniMax music generation')
  }
}
