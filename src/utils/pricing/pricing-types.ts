...
export type ExtractStepEstimate = ProviderModelBase<'tesseract' | 'ocrmypdf' | 'paddle-ocr' | 'mistral' | 'glm' | 'kimi' | 'openai' | 'anthropic' | 'gemini' | 'deepinfra' | 'firecrawl' | 'gcloud-docai' | 'aws-textract' | 'deapi'> & {
  step: 'extract'
...
export type ComputeEstimatedProcessingTimesInput = {
  ...
  deepinfraOcrModel?: string | undefined
  deapiOcrModel?: string | undefined
  extractTargets?: Array<{ provider: 'mistral' | 'glm' | 'kimi' | 'openai' | 'anthropic' | 'gemini' | 'deepinfra' | 'firecrawl' | 'gcloud-docai' | 'aws-textract' | 'deapi', model: string, pageCount?: number }> | undefined
  extractPageCount?: number | undefined
  llmTargets?: Array<{
    service: Step3Metadata['llmService']
    model: string
