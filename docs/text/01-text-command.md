...
```bash
bun as -- --info --urls "input/example-urls.md"
```
...
```bash
bun as -- --info --playlist "https://www.youtube.com/playlist?list=PLCVnrVv4KhXPz0SoAVu8Rc1emAdGPbSbr"
```
...
```bash
bun as -- text --rss "input/feed.xml"
```
...
```bash
bun as -- text --rss "https://feeds.transistor.fm/fsjam-podcast" --item \
...
```bash
bun as -- text --rss "https://ajcwebdev.substack.com/feed" --info

# Combine all metadata from multiple feeds into a single JSON file
bun as -- text --rss "https://ajcwebdev.substack.com/feed" "https://feeds.transistor.fm/fsjam-podcast" --info combined
```

Process multiple RSS feeds:

```bash
bun as -- text --rss "https://ajcwebdev.substack.com/feed" "https://feeds.transistor.fm/fsjam-podcast"
```

Process multiple RSS feeds listed in a `.md` file:

```bash
bun as -- text --rss "input/example-rss-feeds.md" --last 2
```

Download episodes from a specific number of previous days, for example to download episodes from the last 7 days:

```bash
bun as -- text --rss "https://ajcwebdev.substack.com/feed" --days 7
```

Download episodes from a specific date:

```bash
bun as -- text --rss "https://ajcwebdev.substack.com/feed" --date 2021-05-10
```

Download episodes from multiple dates:

```bash
bun as -- text --rss "https://ajcwebdev.substack.com/feed" --date 2021-05-10 2022-05-10
```
...
These commands automate processing for content organized within the `input/workflows` subdirectory.

Generate shownotes for AI category feeds (replace `YYYY-MM-DD` with the desired date, or omit for default behavior):

```bash
bun as -- text --rss --feed "01-ai-feeds.md" --date 2025-11-01
```

Generate shownotes and info files for AI category feeds:

```bash
bun as -- text --rss --feed "01-ai-feeds.md" --date 2025-08-09 2025-08-10 --metaInfo
```

Generate shownotes for Web category feeds:

```bash
bun as -- text --rss --feed "02-web-feeds.md" --date 2025-08-09 2025-08-10
```

Generate shownotes and info files for Web category feeds:

```bash
bun as -- text --rss --feed "02-web-feeds.md" --date 2025-08-09 2025-08-10 --metaInfo
```