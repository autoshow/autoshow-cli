...
- `test/test-cases/e2e/step-2-ocr-e2e/ocr-services/service-models.test.ts` covers PDF and image extraction with `--mistral mistral-ocr-2512`, `--glm glm-ocr`, `--kimi kimi-k2.6`, `--openai gpt-5.4-nano`, `--anthropic claude-haiku-4-5`, `--gemini gemini-3.1-flash-lite-preview`, and `--deepinfra Qwen/Qwen3-VL-30B-A3B-Instruct`.
...
`ocr-glm-reader.test.ts` currently has no mapped price command, so `--test-price` does not add a command for it and `--budget` does not skip it.
