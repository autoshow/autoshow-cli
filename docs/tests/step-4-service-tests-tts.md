...
- `test/test-cases/validation/tts-provider-contracts.test.ts` covers OpenAI instructions/speed, Grok language/text-normalization and custom voice IDs, Groq Arabic default voice selection, MiniMax synthesis controls, Hume Octave file requests and voice payloads, Cartesia byte synthesis requests, and provider-specific mocked request metadata.
...
The Kitten pipeline price selector is `test/test-price/step-4-tts/pipeline`; budget preflight still maps the live e2e file.
