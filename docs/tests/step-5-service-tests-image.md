...
- `openai-image-gen.test.ts` also covers `gpt-image-2` low-quality `1024x1024`, JPEG output options, multi-provider `--price` output, and `write` pipeline integration with `--image openai=...`.
...
bun t test/test-cases/e2e/service/step-5-image-gen-e2e/openai-image-gen.test.ts --budget 2500
```
