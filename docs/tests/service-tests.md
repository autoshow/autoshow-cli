...
These commands are opt-in service coverage. They may reach OpenAI, Anthropic, Gemini, Mistral, AWS, Google Cloud, ElevenLabs, MiniMax, Hume, Cartesia, deAPI, Reve, Deepgram, Groq, Grok, Firecrawl, or other paid providers when credentials are present. Agents should use `bun run check` plus the no-cost targeted tests in [Local Tests](local-tests.md) unless the user explicitly approves live or full-suite testing.
