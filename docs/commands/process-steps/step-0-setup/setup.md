...
Reports the status of required tools (yt-dlp, ffmpeg, ffprobe, tesseract), API keys (including hosted extract keys such as `MISTRAL_API_KEY`, `GLM_API_KEY`, and `FIRECRAWL_API_KEY`, hosted STT keys such as `DEEPINFRA_API_KEY` and `DEAPI_API_KEY`, and generation keys such as `XAI_API_KEY`, `RUNWAYML_API_SECRET`, `BFL_API_KEY`, and `MINIMAX_API_KEY`), Google Cloud STT + Document AI OCR gcloud readiness, AWS CLI Transcribe readiness, config file validity, and Bun version.
