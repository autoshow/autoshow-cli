...
- `--kimi` uses Kimi's OpenAI-compatible chat API with JSON mode, disables K2.6 thinking for predictable write latency, and still validates results against AutoShow's structured output schema.
...
| `--kimi <model>` | Select one or more Kimi models; omit the value to use the cheapest supported model |
...
- `write` accepts the same step-2 OCR and STT flags documented in [`extract STT`](../step-2-extract/02-extract-stt.md#stt-flags) and [`extract OCR`](../step-2-extract/03-extract-ocr.md#ocr-flags), including `--gcloud-stt`, `--happyscribe-stt`, and `--happyscribe-organization-id`, but each `write` run may select at most one STT provider and at most one OCR provider.
