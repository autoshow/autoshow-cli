...
| Flag | Description |
|------|-------------|
| `--all-providers llm` | Enable every supported LLM provider/model for this command |
| `--batch-concurrency <n>` | Batch items to process concurrently; default `1` |
| `--provider-concurrency <n>` | Hosted providers/models to run concurrently per write item; default `2` |
| `--local-concurrency <n>` | Local providers/models to run concurrently per write item; default `1` |
...
```bash
bun as write https://ajc.pics/autoshow/examples/1-audio.mp3 --llm openai=gpt-5.4 --prompt shortSummary longSummary
...
Step selectors accept `provider[=model]`. Omitting the model resolves to the cheapest supported model for that provider. Model-selecting flags are repeatable, including repeated selectors from the same provider.
...
```bash
bun as write https://ajc.pics/autoshow/examples/1-audio.mp3 --llm llama
bun as write https://ajc.pics/autoshow/examples/1-audio.mp3 --llm llama=ggml-org/Qwen3-0.6B-GGUF
bun as write input/examples/document/1-document.pdf --llm llama=ggml-org/gemma-3-270m-it-GGUF
bun as write input/examples/document/1-epub.epub --epub-bun --llm llama --format json
```
...
bun as write https://ajc.pics/autoshow/examples/1-audio.mp3 --llm openai=gpt-5.4-nano
bun as write https://ajc.pics/autoshow/examples/1-audio.mp3 --stt deepgram --llm openai=gpt-5.4
bun as write https://ajc.pics/autoshow/examples/1-audio.mp3 --llm openai=gpt-5.4 --llm openai=gpt-5.4-mini
bun as write ./output/demo/text/01-track-one.md --llm openai=gpt-5.4 --prompt folkSong
```

Passing `--llm openai` keeps the existing cheapest-model default; `gpt-5.5` is used only when selected explicitly or through `--all-providers llm`. `gpt-5.5-pro` is not listed because this CLI change adds the requested `gpt-5.5` model.
...
```bash
bun as write https://ajc.pics/autoshow/examples/1-audio.mp3 --llm anthropic=claude-opus-4-7
```
...
```bash
bun as write https://ajc.pics/autoshow/examples/1-audio.mp3 --llm groq=openai/gpt-oss-20b
```
...
```bash
bun as write https://ajc.pics/autoshow/examples/1-audio.mp3 --llm minimax=MiniMax-M2.7
```
...
bun as write https://ajc.pics/autoshow/examples/1-audio.mp3 --llm grok=grok-4.20-non-reasoning
```

Passing `--llm grok` preserves the previous Grok default, `grok-4.20-non-reasoning`. `grok-4.3` is used only when selected explicitly or through `--all-providers llm`.
...
```bash
# Multi-provider run
bun as write https://ajc.pics/autoshow/examples/1-audio.mp3 --llm openai=gpt-5.5 --llm grok=grok-4.3 --llm groq=openai/gpt-oss-20b --llm glm=glm-5.1 --llm kimi=kimi-k2.6
```
