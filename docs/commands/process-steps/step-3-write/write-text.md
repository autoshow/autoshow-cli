...
- `write` accepts the same step-2 OCR and STT flags documented in the [`extract` command](../step-2-extract/extract.md), including `--gcloud-stt`, `--happyscribe-stt`, and `--happyscribe-organization-id`, but each `write` run may select at most one STT provider and at most one OCR provider.
