...
- `write` accepts the same step-2 OCR and STT flags documented in the [`extract` OCR path](../step-2-ocr/ocr-document.md) and [`extract` STT path](../step-2-stt/stt-audio.md), including `--gcloud-stt`, `--happyscribe-stt`, and `--happyscribe-organization-id`, but each `write` run may select at most one STT provider and at most one OCR provider.
