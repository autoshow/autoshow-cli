...
|------|----------------|-------------|
| Hosted music generation | Prompt or `.md` / `.txt` file plus `--elevenlabs-music`, `--minimax-music`, or `--all-music` | Calls hosted music APIs and writes audio files |
...
- `ELEVENLABS_API_KEY` and `MINIMAX_API_KEY` status
- `ffmpeg` and `ffprobe`
